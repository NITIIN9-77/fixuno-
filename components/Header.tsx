...
const Header: React.FC<HeaderProps> = ({ onOpenHistory, onNavigate }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? 'bg-surface/95 shadow-2xl backdrop-blur-md' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <button onClick={toggleMobileMenu} className="lg:hidden p-2 text-textSecondary z-[60]">
      ...
      <div className={`fixed top-0 right-0 bottom-0 w-[85%] max-w-sm bg-surface z-50 lg:hidden transition-transform duration-500 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] border-l border-slate-800 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
            {/* Drawer Header to avoid overlap */}
            ...
            <div className="flex-1 overflow-y-auto px-8 py-8 space-y-6">
                ...
                <div className="relative mb-8">
                    <input 
                        ...
                        className="w-full bg-slate-800/50 border border-slate-700 rounded-2xl px-5 py-4 text-textPrimary focus:outline-none focus:border-primary shadow-inner"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                    />
                </div>
                ...
                <div className="flex flex-col space-y-2">
                    <button onClick={() => handleNavClick('/')} className="text-2xl font-black text-textPrimary text-left py-3 uppercase tracking-tighter border-b border-slate-800/50 hover:text-primary transition-colors">HOME</button>
                    
                    {/* Collapsible Mobile Services */}
                    <div>
                        <button 
                            onClick={() => setIsMobileServicesOpen(!isMobileServicesOpen)}
                            className="w-full text-2xl font-black text-textPrimary text-left py-3 uppercase tracking-tighter border-b border-slate-800/50 flex justify-between items-center group hover:text-primary transition-colors"
                        >
                            SERVICES
                            <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 transition-transform ${isMobileServicesOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div className="grid grid-cols-1 gap-3 pl-4">
                                {SERVICES.map(s => (
                                    <button 
                                        ...
                                        className="text-left py-3 px-4 bg-slate-800/30 rounded-xl text-textSecondary font-bold hover:text-primary hover:bg-primary/10 transition-all border border-slate-800"
                                    >
                                        {s.name}
                                    </button>
                                ))}
                    ...
                    <button onClick={() => handleNavClick('/reviews')} className="text-2xl font-black text-textPrimary text-left py-3 uppercase tracking-tighter border-b border-slate-800/50 hover:text-primary transition-colors">REVIEWS</button>
                    <button onClick={() => handleNavClick('/contact-us')} className="text-2xl font-black text-textPrimary text-left py-3 uppercase tracking-tighter border-b border-slate-800/50 hover:text-primary transition-colors">CONTACT</button>
                </div>

                {/* Call to Action in Menu */}
                <div className="pt-10">
                    <button 
                        onClick={() => { onOpenHistory?.(); toggleMobileMenu(); }} 
                        className="w-full bg-primary/10 text-primary border border-primary/30 py-4 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-primary hover:text-white transition-all shadow-xl"
                    >
                        MY BOOKINGS
                    </button>
                    <a href="tel:8423979371" className="mt-6 flex items-center justify-center space-x-3 text-textSecondary hover:text-primary transition-colors py-4">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg>
                        <span className="font-black text-sm">Call Expert: 8423979371</span>
                    </a>
                </div>
            </div>
            
            <div className="p-8 border-t border-slate-800/50 text-center">
                <p className="text-[10px] text-textSecondary uppercase tracking-[0.3em] font-black">All Day Open • 24/7 Support</p>
            </div>
        </div>
      </div>
    </header>
