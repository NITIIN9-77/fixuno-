...
const BookingHistoryModal: React.FC<BookingHistoryModalProps> = ({ bookings, onClose }) => {
  useEffect(() => {
    ...
    <div className="fixed inset-0 bg-background/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-surface rounded-xl shadow-2xl w-full max-w-2xl m-4 relative animate-slide-in-up flex flex-col max-h-[80vh] border border-slate-700" onClick={(e) => e.stopPropagation()}>
        <header className="p-6 border-b border-slate-700 flex justify-between items-center bg-surface sticky top-0 z-10 rounded-t-xl">
          <div>
            <h2 className="text-2xl font-bold text-textPrimary">Backend Records</h2>
            <p className="text-sm text-textSecondary">View all your synchronized bookings.</p>
          </div>
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-6">
          {bookings.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-textSecondary">No bookings found in the database.</p>
              <button onClick={onClose} className="mt-4 text-primary hover:underline font-medium">Book your first service now</button>
            </div>
          ) : (
            bookings.map((booking) => (
              <div key={booking.id} className="bg-background/40 border border-slate-800 rounded-lg p-5 hover:border-primary/30 transition-all">
                ...
                <div className="space-y-2 border-t border-slate-800 pt-4 mt-4">
                  {booking.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-sm">
                      ...
                      <span className="text-textPrimary">₹{item.price * item.quantity}</span>
                    </div>
                  ))}
